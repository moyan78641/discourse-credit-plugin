...
class CreditWalletPage extends Component {
  ...
  @action nextPage() { if (this.page * 20 < this.totalOrders) { this.page++; this.loadOrders(); } }
      ...
      {{else if this.wallet}}
        ...
        {{#if this.needsPayKey}}
          <div class="credit-paykey-required">
            ...
            <div class="paykey-form">
              ...
              <button class="btn btn-primary btn-large" type="button" disabled={{this.payKeySaving}} {{on "click" this.savePayKey}}>
                {{if this.payKeySaving "设置中..." "确认设置"}}
              </button>
            </div>
          </div>
        ...
        {{else}}
          ...
          <div class="credit-wallet-card">
            ...
            <div class="wallet-leaderboard-info">
              <span>{{icon "chart-line"}} 排行榜基准分: {{this.wallet.initial_leaderboard_score}}</span>
