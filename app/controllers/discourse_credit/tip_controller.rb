...
module ::DiscourseCredit
  class TipController < BaseController
    ...
    skip_before_action :ensure_logged_in, only: [:post_tips, :batch_post_tips]
    ...
    # GET /credit/tip/posts.json?post_ids[]=1&post_ids[]=2 — 批量获取多个帖子的打赏记录
    def batch_post_tips
      post_ids = Array(params[:post_ids]).map(&:to_i).uniq.first(50) # 最多50个
      return render json: {} if post_ids.empty?

      tips = CreditOrder.where(order_type: "tip", post_id: post_ids, status: "success").order(created_at: :desc)
      ...
      result = {}
      grouped.each do |pid, post_tips|
        total = post_tips.sum { |t| t.actual_amount.to_f }
        result[pid.to_s] = {
          total_amount: total,
